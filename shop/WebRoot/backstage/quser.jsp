		...
		<div class="row-fluid">
			...
			<div>
				<ul class="breadcrumb">
					...
					<li>
						<a href="/shop/QUServlet?id=21">用户查看</a>
					</li>
				</ul>
			</div>
			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-align-justify"></i> 用户查看</h2>
						<div class="box-icon">
					...
					<div style="height:30px; width:1000px;">
						...
						<div style="width:450px; float:right; margin-top: 10px; ">
							<form action="/shop/QUNameServlet" method="post">
								<span style="color:black;font-size:14px;">查询(按用户名字):</span>
						...
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
						  	...
						  	<tr>
						  		...
						  		<td><%=list.get(i).getuPsw() %></td>
						  		<%if(list.get(i).getuStatus() == 1){ %>
						  		<td>可用</td>
						  		<%}else{ %>
						  		<td>停用</td>
						  		<%} %>
						  		<td>
						  			<a class="btn btn-success" href="/shop/QueryCommodityIdServlet?cId=<%=list.get(i).getuId() %>">
										<i class="icon-zoom-in icon-white"></i>  
										详细                                            
									</a>
									<a class="btn btn-danger" href="/shop/StopUServlet?uName=<%=list.get(i).getuName() %>&uStatus=<%=list.get(i).getuStatus() %>">
										<%if(list.get(i).getuStatus() == 1){ %>
							...
							<tr>
						  		...
						  		<%} %>
						  		<td>
						  			<a class="btn btn-success" href="/shop/QUIdServlet?uName=<%=list.get(i).getuName() %>">
										<i class="icon-zoom-in icon-white"></i>  
										详细                                            
									</a>
									<a class="btn btn-danger" href="/shop/StopUServlet?uName=<%=list.get(i).getuName() %>&uStatus=<%=list.get(i).getuStatus() %>">
					  ...
					  <div class="pagination pagination-centered">
						  <ul>
							...
							<%for(int i=1;i<((listsize/10)+2);i++){ %>
								<li><a href="/shop/QUFServlet?num=<%=i %>">[<%=i %>]</a></li>
							<%} %>
