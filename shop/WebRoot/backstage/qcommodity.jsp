		...
		<div class="row-fluid">
			...
			<div>
				<ul class="breadcrumb">
					...
					<li>
						<a href="/shop/QueryCommodityServlet?id=11">商品查看</a>
					</li>
				</ul>
			</div>
			...
			<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-calendar"></i> 商品查看</h2>
						<div class="box-icon">
					...
					<div style="height:30px; width:1000px;">
						...
						<div style="width:450px; float:right; margin-top: 10px; ">
							<form action="/shop/QCommodityNameServlet" method="post">
								<span style="color:black;font-size:14px;">查询(按商品编号):</span>
								<input type="text" style="width:200px;" name="cNum">
							</form>
						</div>
					</div>
					<div class="box-content">
						<table class="table table-striped table-bordered bootstrap-datatable datatable">
						  ...
						  <tbody>
							...
							%>
							<tr>
								<td><%=list1.get(i).getcNum() %></td>
								<%for(int k=0;k<list.size();k++){
									if(list1.get(i).getcNum().equals(list.get(k).getcNum())){
										cName = list.get(k).getcName();
									}
								}
								 %>
								<td><%=cName %></td>
								<td class="center"><%=list1.get(i).gethInC()%></td>
								<td class="center"><%=list1.get(i).gethOutC()%></td>
								<td class="center"><%=(list1.get(i).gethInC() - list1.get(i).gethOutC())%></td>
								<td class="center">
									<form action="/shop/JHServlet" method="post">
										<input type="text" style="width:75px;" name="innum"/>
										<input type="hidden" name="cNum" value="<%=list1.get(i).getcNum() %>"/>	
									</form>									
								</td>
							</tr>
							<%} 
							...
							<tr>
								...
								<td class="center">
									<form action="/shop/JHServlet" method="post">
										<input type="text" style="width:75px;" name="innum"/>
										<input type="hidden" name="hid" value="<%=list1.get(i).gethId() %>"/>	
									</form>							
								</td>
							</tr>
							<%} 
					  ...
					  <div class="pagination pagination-centered">
						  <ul>
							...
							<%for(int i=1;i<((listsize/10)+2);i++){ %>
								<li><a href="/shop/QCommodityFServlet?num=<%=i %>">[<%=i %>]</a></li>
							<%} %>
